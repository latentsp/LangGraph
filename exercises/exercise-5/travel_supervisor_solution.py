...
budget_agent = create_react_agent(
    model=model,
    tools=[calculate_trip_cost, compare_costs],
    name="budget_expert",
    prompt="You are a travel budget expert. Always use one tool at a time."
)

planner_agent = create_react_agent(
    model=model,
    tools=[plan_trip_itinerary, suggest_activities],
    name="trip_planner",
    prompt="You are a trip planning expert. Always use one tool at a time."
)
...
workflow = create_supervisor(
    ...
    model=model,
    prompt=(
        "You are a travel supervisor managing a budget expert and a trip planner. "
        "For cost questions, use budget_expert. "
        "For itinerary and activity planning, use trip_planner."
    )
)
...
result = app.invoke({
    "messages": [
        {
            "role": "user",
            "content": "Plan a 5-day trip to Paris for 2 people and tell me how much it will cost"
        }
    ]
})
print(result) 